...
const AP_Param::GroupInfo AP_OSD_Screen::var_info[] = {
    ...
    // @Description: Vertical position on screen
    // @Range: 0 15
    AP_SUBGROUPINFO(energy, "ENERGY", 61, AP_OSD_Screen, AP_OSD_Setting),
...
void AP_OSD_Screen::draw_aspeed(uint8_t x, uint8_t y)
{
    ...
    if (have_estimate) {
        backend->write(x, y, aspd < osd->warn_aspd, "%c%4d%c", SYM_ASPD, (int)u_scale(SPEED, aspd), u_icon(SPEED));
    } else {
...
void AP_OSD_Screen::draw_climbeff(uint8_t x, uint8_t y)
{
    ...
    if (is_positive(vspd)) {
        ...
        if (efficiency < 1000) {
            backend->write(x, y, false,"%c%c%3lu%c", SYM_PTCHUP, SYM_EFF, efficiency, unit_icon);
            return;
        }
    }
invalid:
